...
func (p Parallelogram) CalculatePeak(d [3]Dot) Dot {
	center := p.FindCenter()
...
func (p Parallelogram) CalculateArea() float64 {
	side_a := math.Sqrt(float64(math.Pow(float64(p.peak[0].x-p.peak[1].x), 2)) + math.Pow(float64(p.peak[0].y-p.peak[1].y), 2))
	side_b := math.Sqrt(float64(math.Pow(float64(p.peak[1].x-p.peak[2].x), 2)) + math.Pow(float64(p.peak[1].y-p.peak[2].y), 2))
	diag := math.Sqrt(float64(math.Pow(float64(p.peak[0].x-p.peak[2].x), 2)) + math.Pow(float64(p.peak[0].y-p.peak[2].y), 2))
